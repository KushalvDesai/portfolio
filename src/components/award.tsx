...
export function Awards({
  ...
  if (variant === "award") {
    const LaurelWreath = () => (
      <svg
        className={cn(
          "fill-primary absolute top-1/2 h-full w-full -translate-y-1/2 opacity-20 dark:opacity-40"
        )}
        width="892.77"
        height="688.08"
    ...
    return (
      <div
        className={cn(
          "relative z-0",
          "flex flex-col items-center justify-center p-4 min-h-[300px]",
          "overflow-hidden bg-transparent",
          className
        )}
      >
        <LaurelWreath />
        <div className="z-10 px-8 text-center mt-6">
          {level !== "none" && (
            <div
              className={cn(
                "mb-2 inline-block rounded-md px-4 py-1 tracking-wider text-white",
          ...
          {/* Decorative Line */}
          <div className="bg-primary mx-auto my-3 h-[1px] w-20"></div>

          {/* Subtitle */}
          <h2 className={cn("mb-2 w-full max-w-[200px] mx-auto text-lg font-light leading-snug")}>{subtitle}</h2>

          {/* Recipient */}
          {recipient && (
            <p className={cn("text-primary/60 italic text-sm")}>{recipient}</p>
          )}

          {/* Date */}
          <div className={cn("text-md font-bold mt-2")}>{date}</div>
        </div>
      </div>
    )
  }
